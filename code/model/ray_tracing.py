...
class RayTracing(nn.Module):
    ...
    def forward(self,
        ...
        end_time = time.time()
        print("Time taken for get_sphere_intersection: ", end_time - start_time)
        ...
        print("Time taken for sphere_tracing: ", end_time - start_time)
        ...
        if sampler_mask.sum() > 0:
            ...
            end_time = time.time()
            print("Time taken for ray_sampler: ", end_time - start_time)
        ...
        if mask.sum() > 0:
            ...
            end_time = time.time()
            print("Time taken for minimal_sdf_points: ", end_time - start_time)
    ...
    def ray_sampler(self, sdf, cam_loc, object_mask, ray_directions, idx, sampler_min_max, sampler_mask):
        ...
        # print('points original shape: ',points.shape )
        idx_expand = idx.unsqueeze(-1).expand(-1, -1, self.n_steps).reshape(-1, self.n_steps)
        end_time = time.time()
        print("Time taken for 初始化采样点: ", end_time - start_time)
        ...
        # 在上述光线的所有采样点处，以批量方式计算 SDF 函数的值。
        sdf_val_all = []
        # print('points shape in sdf: ',points.shape)
        for pnts, i in zip(torch.split(points.reshape(-1, 3), 300000, dim=0), torch.split(idx_expand.reshape(-1), 300000, dim=0)):
            # print('split pnts shape  ', pnts.shape)
            # print('split pnts ', pnts)
            sdf_val_all.append(sdf(pnts, i))
        sdf_val = torch.cat(sdf_val_all).reshape(-1, self.n_steps)
        end_time = time.time()
        print("Time taken for 处理非收敛光线: ", end_time - start_time)
        ...
        end_time = time.time()
        print("Time taken for 处理最小SDF值: ", end_time - start_time)
        ...
        #     start_time = time.time()
        #     # 如果存在需要使用 secant 方法的像素点，将运行 secant 方法对其进行处理。Secant 方法是一种用于找到函数零点的数值方法，非常适用于在 SDF 中找到物体表面
        #     z_pred_secant = self.secant(sdf_low, sdf_high, z_low, z_high, cam_loc_secant, ray_directions_secant, idx_secant, sdf)
        #     end_time = time.time()
        #     print("Time taken for secant: ", end_time - start_time)
        #     # Get points
    ...
    def secant(self, sdf_low, sdf_high, z_low, z_high, cam_loc, ray_directions, idx, sdf):
        ...
        print('z_hight: ', z_high)
        print('z_low: ', z_low)
        print('sdf_high: ', sdf_high)
        print('sdf_low: ', sdf_low)
        print('z_pred: ', z_pred)

        for i in range(self.n_secant_steps):
            p_mid = cam_loc + z_pred.unsqueeze(-1) * ray_directions
            # print('secant p_mid: ',p_mid)
            # print('secant more')
    ...
    def minimal_sdf_points(self, num_pixels, sdf, cam_loc, ray_directions, idx, mask, min_dis, max_dis):
        ...
        mask_sdf_all = []
        for pnts, i in zip(torch.split(points, 300000, dim=0), torch.split(mask_idx, 300000, dim=0)):
