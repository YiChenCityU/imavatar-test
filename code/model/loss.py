...
class Loss(nn.Module):
    ...
    def get_mask_loss(self, sdf_output, network_object_mask, object_mask, valid_mask):
        ...
        print('object_mask size: ',object_mask.shape)
    ...
    def get_eikonal_loss(self, normal):
        eikonal_loss = F.mse_loss(normal, normal.new_ones(normal.shape), reduction='mean')
        return eikonal_loss
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        # sdf_loss = self.get_sdf_consistency_loss(model_outputs['sdf_values'])

        eikonal_loss = self.get_eikonal_loss(model_outputs['normal_values'])
        print('eikonal_loss: ', eikonal_loss)
        loss = rgb_loss + self.mask_weight * mask_loss + 0.1 * eikonal_loss
